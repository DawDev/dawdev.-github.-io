...
class Room {
    constructor() {
                        // just in case ik geen bepaalde functie nodig heb en vergeet om die te implementeren en leeg laten.
                        // Probably super onnodig maar it is what it is.
    }
...
export class RoomOne extends Room{        // Use inheritance perhaps? a @TODO?
    constructor(game) {
        super(); // init de parent class
        this.game = game; // reference naar de game object
        this.assets = {
...
class Terminal extends Room{
    constructor(game) {
        ...
        this.cableStartDrag = [
            ...
            new Rect(255, 50, 100, 100), // De 'colliders' die de player kan clicken om te beginnen met een kabel draggen
            new Rect(420, 50, 100, 100),
        ];

        this.cableConnectFrom = [
            new Rect(115, 90, '_', '_'), // de kabels zijn niks meer dan canvas lines, dus je tekent een line vanaf een van deze punten
            new Rect(300, 90, '_', '_'),
            new Rect(465, 90, '_', '_'),
        ]

        this.cableConnectTo = [
            new Rect(105, 480, '_', '_'), // en tekent de line "tot" hier.
            new Rect(300, 480, '_', '_'),
            new Rect(485, 480, '_', '_'),
        ]

        this.cableStopDrag = [
            new Rect(50, 430, 120, 100), // De 'colliders' waar de player de mouseknop kan declicken om de kabel te verbinden
            new Rect(225, 430, 140, 100),
            new Rect(425, 430, 120, 100)
        ]

        this.draggingCable = false; // spreekt voor zich
        this.draggedCable = -1; // de currently draggen kabel index
    }
    ...
    checkWin() {
        this.connections.forEach((c) => {
            ...
            if (!includes) { // checked of de kabels goed verbonden zijn door een kabel connectie lookup
                this.wrong = true;
        ...
        this.cableStartDrag.forEach((t, i) => {
            if (Tools.pointInRect(e.offsetX, e.offsetY, t)) {
                if (!this.cablesConnected.includes(i)) {
                    ...
                    this.draggedCable = i; // spreekt voor zich
                }
            }
        })
    }
...
export class RoomThree extends Room {
    ...
    update() {
        ...
        if (this.liquidVileColliders.length == 0) {
            if (Tools.AABB(this.player, this.tableCollider) && !this.madeAcid) {
